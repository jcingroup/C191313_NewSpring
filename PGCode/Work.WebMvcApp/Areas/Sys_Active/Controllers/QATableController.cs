...
{
    public class QATableController : BaseAction<m_QATable, a_QATable, q_QATable, QATable>
    {
        ...
        public override ActionResult ListGrid(q_QATable sh)
        {
        ...
        public override ActionResult EditMasterNewData()
        {
            ...
            ac = new a_QATable() { Connection = getSQLConnection(), logPlamInfo = plamInfo };
            md.Id = ac.GetIDX();
            md.IsOpen = true;
        ...
        public override ActionResult EditMasterDataByID(int id)
        {
            ...
            RunOneDataEnd<m_QATable> HResult = ac.GetDataMaster(id, LoginUserId);
        ...
        [HttpPost]
        [ValidateInput(false)]
        public String ajax_MasterUpdata(m_QATable md)
        {
        ...
        /// <returns></returns>
        [HttpGet]
        public override String ajax_MasterGridData(q_QATable queryObj)
        {
            #region 連接BusinessLogicLibary資料庫並取得資料
            ac = new a_QATable() { Connection = getSQLConnection(), logPlamInfo = plamInfo };
            RunQueryPackage<m_QATable> HResult = ac.SearchMaster(queryObj, LoginUserId);
            HandleResultCheck(HResult);
            #endregion
